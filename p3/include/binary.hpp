...
class number<N,2,T> {
    ...
    public:
        ...
        number<N,2,T> operator+(const number<N,2,T>& n) const;
        number<N,2,T> operator-(const number<N,2,T>& n) const;
...
template <int N, typename T>
number<N, 2, T> number<N, 2, T>::suma(const number<N,2,T> &s) const {
...
template <int N, typename T>
number<N, 2, T> number<N, 2, T>::resta(const number<N,2,T> &s) const {
